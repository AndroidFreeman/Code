...
class LocalProfiles {
  ...
  static Future<void> saveAutoLogin({
    ...
    try {
      final f = File(p.join(dataDir, 'auto_login.json'));
  ...
  static Future<Profile?> loadAutoLoginProfile({
    ...
    try {
      final payload = jsonDecode(await f.readAsString(encoding: utf8));
      if (payload is Map &&
        ...
        ).copyWith(position: payload['position'] ?? '');
      }

      final pid =
          (payload is Map ? payload['profile_id'] : null).toString().trim();
  ...
  static Profile? loadAutoLoginProfileFast({
    ...
    try {
      ...
      final payload = jsonDecode(f.readAsStringSync(encoding: utf8));
      if (payload is Map &&
          payload.containsKey('id') &&
          payload.containsKey('role')) {
        return Profile(
      ...
      // Fallback to old behavior: if it only has profile_id, we need to load from CSV
      final pid =
          (payload is Map ? payload['profile_id'] : null).toString().trim();
      if (pid.isEmpty) return null;
