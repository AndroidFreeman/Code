/*
 * @Date: 2026-03-26 19:32:11
 * @Github: https://github.com/AndroidFreeman
 * Now, I use my Codespace
 * @Author: Android_Freeman
 * @LastEditTime: 2026-03-27 14:27:57
 * @FilePath: /Code/_CProgram/insert.cpp
 */

#include <bits/stdc++.h>
using namespace std;
/**
 * 给后端组的提示：
 * 1. 不要用 scanf()！因为 Flutter 没法在手机屏幕里给你打字。
 * 2. 所有的输入都通过 argc(参数个数) 和 argv(参数内容) 传进来。
 * 3. 所有的输出必须是 printf() 打印出来的 JSON 字符串。
 */

int main(int argc, char* argv[]) {
    // ---- 第一步：检查参数够不够 ----
    // argv[0] 是程序名自己，argv[1] 是第一个参数，以此类推
    if (argc < 6) {
        // 如果参数不够，打印一个错误的 JSON 告诉前端
        printf(
            "{\"status\":\"error\", \"message\":\"参数不足!需要:学生ID"
        "姓名  性别 所属学院 登录密码\"}\n");
        return 1;  // 返回 1 表示出错了
    }

    // ---- 第二步：接收前端传来的数据 ----
    char* name = argv[1];  // 比如 "张三"
    char* sex= argv[2];   // 比如 "20"
    char*IDnum=argv[3];
    char*school=argv[4];
    char*password=argv[5];
    // ---- 第三步：执行你的逻辑（比如写文件、计算等） ----
    // 这里是你的核心代码，比如把数据存进 CSV 文件
    FILE* file = fopen("database.csv", "a");  // 以“追加”模式打开文件
    if (file == NULL) {
        printf("{\"status\":\"error\", \"message\":\"无法打开数据库文件\"}\n");
        return 1;
    }

    // 写入信息
    fprintf(file, "%s,%s,%s,%s,%s\n", name, sex,IDnum,school,password);
    fclose(file);  // 记得关文件

    // ---- 第四步：大功告成，反馈给前端 ----
    // 💡 注意：JSON 里的双引号在 C 语言里要写成 \"
    // 这一行打印出来，Flutter 就能抓到并显示在 UI 上
    printf(
        "{\"status\":\"success\", \"message\":\"学生注册成功\", "
        "\"IDnum\":\"%s\",\"name\":\"%s\",\"school\":\"%s\"}\n",
       IDnum, name,school);

    return 0;  // 返回 0 表示完美执行
}
