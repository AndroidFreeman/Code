...
int main(int argc, char **argv) {
  ...
  // Validate inputs: reject commas to prevent CSV corruption
  auto contains_comma = [](const char* s) {
    return s && std::strchr(s, ',') != nullptr;
  };
  if (contains_comma(id) || contains_comma(name) || contains_comma(teacher_id) ||
      contains_comma(term) || contains_comma(color) || contains_comma(credits) ||
      contains_comma(notes)) {
    json_err("bad_request", "Field values must not contain commas");
    return 1;
  }
