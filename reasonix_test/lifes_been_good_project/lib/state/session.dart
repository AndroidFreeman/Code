...
class Session extends ChangeNotifier {
  ...
  Future<void> preloadAll() async {
    ...
    try {
      ...
      if (await features.hasFeature('profiles_list')) {
        futures.add(
            features.csvOp(action: 'read', file: 'profiles.csv').then((res) {
      ...
      if (await features.hasFeature('courses_list')) {
        futures.add(DatabaseHelper.instance
            .getCacheData('courses')
            .then((cached) async {
          if (cached != null) {
            preloadedData['courses'] = jsonDecode(cached);
          } else {
            final res =
                await features.csvOp(action: 'read', file: 'courses.csv');
            if (res['ok'] == true) preloadedData['courses'] = res['data'];
          }
        }));
      }

      if (await features.hasFeature('classes_list')) {
        futures.add(
            DatabaseHelper.instance.getCacheData('classes').then((cached) async {
          if (cached != null) {
            preloadedData['classes'] = jsonDecode(cached);
          } else {
            final classesRes =
                await features.csvOp(action: 'read', file: 'classes.csv');
      ...
      if (await features.hasFeature('students_list')) {
        futures.add(
            features.csvOp(action: 'read', file: 'students.csv').then((res) {
      ...
      if (kDebugMode) {
        developer.log('Session.preloadAll error: $e', name: 'session');
      }
    }
  }
  ...
  void logout() {
    // Clear sensitive in-memory state so listeners can detect the change.
    _profile = Profile.empty();
    preloadedData.clear();
    preloadedSignatures.clear();
    stopRealtimeSync();
    notifyListeners();
  }
